# 16.2.3
- рефакторинг репозиториев избранного и visited. Теперь сохраняется весь объект, чтобы потом не ходить за ним в сеть. Ранее сохранялся id.
- добавлено SightRepositoryCached - кеширование запросов на сервер в db. Стратегия - кешируем на 5 минут, координаты округляем до 2 знаков после запятой, чтобы прыгающий gps не давал помехи.
- добавлен map_launcher. Навигация до места по кнопке "Построить маршрут"

# 16.2.2
- добавлена страница map.
- страница select_location для выбора координат на стр. add_sight

# 16.2.1 image_picker
- при добавлении места фотографии можно выбрать из галереи или с камеры
- добавил сжатие фотографии в изоляте, во время сжатия крутится спиннер.
- фотографии заливаются на сервер, во время заливки крутится спиннер
...
- Для сохранения всего фильтра и для упрощения добавил json_serializable
- Работа с сохранением-получением фильтра через change-performer. Показалось переусложнение для такой простой штуки.
Хотя для однородности и порядка, в большом проекте, так было бы наверняка лучше.
- Сделал работу с FirstRun и DarkTheme, через инъекцию репозитория в wm или в интерактор через конструктор.

# 14.6 hero
- добавлен Hero к кнопке "Новое место"
Тут такое дело. В задании нужно добавить Hero между фотографией на карточке и фотографией на детализации.
Но ранее, в каком-то задании, надо было переделать страницу детализации на bottomSheet.
И там весь bottomSheet красиво всплывает снизу и Hero не к месту.
Возможно надо подкорректировать задание?
Поэтому я сделал Hero для кнопки "Новое место". Вроде красивенько. )

# 14.4 implicit animations
